#!/usr/bin/env python3
"""
Logistic Regression Stability Analysis
Runs comprehensive stability analysis for both Y1 and Y2 outcomes using Logistic Regression
"""
import os
import sys
...
def run_stability_analysis(selected_outcome='Y1'):
    """Run stability analysis for specified outcome using Logistic Regression"""
    print(f"\n{'='*60}")
    print(f"STARTING LOGISTIC REGRESSION STABILITY ANALYSIS FOR {selected_outcome}")
    print(f"{'='*60}")
    ...
    # Logistic Regression models to test with different regularization
    models = {
        'LR_L2': LogisticRegression(penalty='l2', C=1.0, max_iter=1000, random_state=42),
        'LR_L1': LogisticRegression(penalty='l1', C=1.0, max_iter=1000, solver='liblinear', random_state=42),
        'LR_ElasticNet': LogisticRegression(penalty='elasticnet', C=1.0, l1_ratio=0.5, max_iter=1000, solver='saga', random_state=42)
    }
    
    print(f"\nStarting stability testing with {N_ITERATIONS} iterations...")
    print("Testing 3 Logistic Regression variants: L2, L1, ElasticNet")
    ...
    for iteration in range(N_ITERATIONS):
        ...
        for model_name, model in models.items():
            ...
            feature_coefficients = model_copy.coef_[0] if hasattr(model_copy, 'coef_') else None
    ...
    # Feature coefficient analysis for L2 model (most stable)
    print(f"\nFeature Coefficient Analysis (L2 Regularization):")
    print("=" * 50)
    l2_results = [(i, coefs, features) for i, (model, coefs, features) in 
                  enumerate(zip(stability_results['model_name'], 
                               stability_results['feature_coefficients'],
                               stability_results['selected_features'])) if model == 'LR_L2' and coefs is not None]
    
    if l2_results:
        # Average coefficient magnitudes for stable features
        coefficient_analysis = defaultdict(list)
        for iteration, coefficients, features in l2_results:
        ...
        print("Top 10 features by coefficient magnitude (L2 model):")
    ...
    print(f"  L2 regularization: Generally most stable")
    print(f"  L1 regularization: Automatic feature selection")
    print(f"  ElasticNet: Balance between L1 and L2")
    if len(core_biomarkers) > 0:
        print(f"  Coefficients provide interpretable feature importance")
    
    # Generate and save visualizations
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    
    # Create directories if they don't exist
    result_num_dir = f'result_num/logistic_regression'
    result_pic_dir = f'result_pic/logistic_regression'
    ...
    axes[0, 0].boxplot(accuracy_data, labels=[m.replace('LR_', '') for m in models_list])
    ...
    for i, model in enumerate(models_list):
        ...
        axes[0, 1].bar(x + i*width, means, width, yerr=stds, 
                      label=model.replace('LR_', ''), alpha=0.8)
    ...
    for model in models_list:
        ...
        cv_data.append(cv)
        model_names.append(model.replace('LR_', ''))
    
    bars = axes[0].bar(model_names, cv_data, alpha=0.8, color=['skyblue', 'lightgreen', 'salmon'])
    axes[0].set_title(f'Regularization Stability - {SELECTED_OUTCOME}\\n(Lower = More Stable)')
    axes[0].set_xlabel('Regularization Method')
    ...
    if results_df['auc_roc'].notna().any():
        ...
        bars2 = axes[1].bar(model_names, auc_data, alpha=0.8, color=['skyblue', 'lightgreen', 'salmon'])
        axes[1].set_title(f'AUC-ROC Comparison - {SELECTED_OUTCOME}')
        axes[1].set_xlabel('Regularization Method')
    ...
    plt.tight_layout()
    plt.savefig(f'{result_pic_dir}/regularization_comparison_{SELECTED_OUTCOME}_{timestamp}.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    # 3. Feature coefficient analysis (L2 model)
    if stable_features_with_coefs:
        ...
        ax.set_title(f'Top Features by Coefficient Magnitude - {SELECTED_OUTCOME}\\n(L2 Regularization)')
        ...
        plt.tight_layout()
        plt.savefig(f'{result_pic_dir}/coefficient_importance_{SELECTED_OUTCOME}_{timestamp}.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    # Save coefficient analysis for top features
    if stable_features_with_coefs:
        coef_analysis_df = pd.DataFrame(stable_features_with_coefs, 
                                       columns=['feature', 'stability_percentage', 'avg_coefficient_magnitude'])
        coef_filename = f'{result_num_dir}/coefficient_analysis_{SELECTED_OUTCOME}_{timestamp}.csv'
        coef_analysis_df.to_csv(coef_filename, index=False)
        print(f"Coefficient analysis saved to: {coef_filename}")
    ...
    with open(report_filename, 'w') as f:
        f.write(f"LOGISTIC REGRESSION STABILITY ANALYSIS REPORT\\n")
        f.write(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\\n")
        f.write(f"Outcome: {SELECTED_OUTCOME}\\n")
        f.write(f"Iterations: {N_ITERATIONS}\\n")
        f.write(f"Models: L2, L1, ElasticNet regularization\\n\\n")
        ...
        if stable_features_with_coefs:
            f.write(f"\\nTOP COEFFICIENT FEATURES (L2 model):\\n")
            for i, (feature, stability, coef_mag) in enumerate(stable_features_with_coefs[:10], 1):
    ...
    print(f"\n{'='*50}")
    print(f"LOGISTIC REGRESSION ANALYSIS FOR {SELECTED_OUTCOME} COMPLETED SUCCESSFULLY!")
...
if __name__ == "__main__":
    print("LOGISTIC REGRESSION STABILITY ANALYSIS")
    ...
    # Final comparison
    print(f"\n{'='*60}")
    print("FINAL COMPARISON SUMMARY - LOGISTIC REGRESSION")
    print(f"{'='*60}")
    ...
    print(f"  - Probabilistic predictions")
    print(f"  - Built-in regularization")
    print(f"  - Handles linear relationships well")
